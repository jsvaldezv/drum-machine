...
function loadPreset() {
    ...
    for (let i = 0; i < presets[indexPresetGlobal].numInstruments * 16; i++) {
        ...
        if (i < 16) padState = presets[indexPresetGlobal].kickPattern[i];
        else if (i >= 16 && i < 32)
...
function createPresets() {
    ...
    presetsContainer.innerHTML +=
        firstPartForm + secondPartForm + thirdPartForm;
...
function getDrumLine(inInstrument) {
    ...
    drumLine.innerHTML =
        "<div class='drumInstrument'>" + inInstrument + "</div>";
...
function createDrumMachine() {
    ...
    if (drumLine != null) {
        for (let i = 0; i < drumLine.length; i++) drumLine[i].remove();
    }
    ...
    if (createdUser) {
        for (let i = 0; i < userSelectChoices.length; i++) {
            ...
            for (let i = 0; i < 16; i++) window.glob.push(false);
        }
        ...
        for (let i = 0; i < numInstruments; i++) {
            drumsContainer.appendChild(
                getDrumLine(presets[indexPresetGlobal].instruments[i])
            );
...
function getUserInstrumentChoice(inIndex) {
    ...
    drumInstrumentChoice.innerHTML = `<form>
...
function createInstrumentChoices() {
    ...
    drumsUserChoice.innerHTML +=
        "<div type='Submit' class='okInstrumentChoices'>Create</div>";
